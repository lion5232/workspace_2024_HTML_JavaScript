 ...
 * 
 ...
 curl "https://openapi.naver.com/v1/search/news.xml?query=%EC%A3%BC%EC%8B%9D&display=10&start=1&sort=sim" \
    -H "X-Naver-Client-Id: Xa5aEIFGSLP3a7plCBW_" \
    -H "X-Naver-Client-Secret: ZGItWcXxJo" -v
*/
...
public class SocketIOEventHandler {
	...
	public SocketIOEventHandler(SocketIOServer server) {
		...
		server.addConnectListener( socket -> {
			...
			//클라로 메시지 전송 (소캣을 통해서 보냄)
			//메시지 형태는 문자열, 구조는 json 혹은 일반 형태도 가능함 지금은 일반 형태)
			//svr_init_msg: 임의의 이벤트 명
			// 서버가 클라이언트에게 쏜다.
		...
		server.addConnectListener(socket -> {
			// socket : 접속 해제한 클라이언트
			System.out.println("클라이언트가 접속 해제 : " + socket.getSessionId().toString());
		});
		//3. 클라이언트가 보낸 커스텀 메시지 처리
		server.addEventListener("cvr_news", String.class, new DataListener<String>() {

			@Override
			public void onData(SocketIOClient client, String data, AckRequest ackSender) throws Exception {
				// TODO Auto-generated method stub
				System.out.println("수신된 메시지 => 뉴스검색결과:  " + naverOpenApiForNews(data));
				// 실습 => 검색결과를 클라에게 전송하기 이벤트명은  "svr_news"
				client.sendEvent("svr_news", naverOpenApiForNews(data)); // 일반 텍스트 형태로 전송
				// 클라이언트는 메시지를 받아서 제목만 추출하여 콘솔에 출력
